...
//     });    
    ...
    router.put("/:id/unfollow", async (req, res) => {
        if(req.body.userId !== req.params.id) {
            try{
                const user = await User.findById(req.params.id);
                const currenntUser = await User.findById(req.body.userId);
                //フォロワーに存在したらフォローを外せる
                if (user.followers.includes(req.body.userId)) {
                    await user.updateOne({
                        $pull: {
                    ...
                    await currenntUser.updateOne({
                        $pull: {
                            followeings: req.params.id,
                        },
                    });
            ...
            return res.status(500).json("自分自身をフォローをフォロー解除できません。");
        }
    });
